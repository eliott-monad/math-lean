...
Partially formalized in `Math/NumberTheory/InvolutionModulus.lean` at math-lean commit
`c949221`: all units modulo `24` square to `1`; `φ(24)=8`; the explicit unit values are
`{1,5,7,11,13,17,19,23}`; coprime naturals satisfy `n^2 % 24 = 1`; and the divisor-side
list `1,2,3,4,6,8,12,24` has exponent-two unit groups.

Still open: the global maximality/classification theorem
`UnitsExponentTwo n ↔ n ∣ 24` for positive `n`.  Exploration notes in
`exploration/2026-06-11-involution-modulus-24.md` identify the likely route: prove descent
of exponent-two unit groups along divisors, then handle odd prime powers and the 2-primary
part locally.
